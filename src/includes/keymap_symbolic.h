 ...
 * same host locale as the TOS.
 *
 * Because the US mapping is a default fallback, it also includes a set of
 * mappings for international keys not present on the US keyboard.
 */
...
static uint8_t Keymap_SymbolicToStScanCode_FR(const SDL_Keysym* pKeySym)
{
	...
	switch (pKeySym->sym)
	{
	 ...
	 case SDLK_ESCAPE: code = 0x01; break;
	 case SDLK_SPACE: code = 0x39; break;
	 case SDLK_EXCLAIM: code = 0x0D; break; /* !§ as ST -_ */
	 ...
	 case SDLK_LEFTPAREN: code = 0x63; break;
	 case SDLK_RIGHTPAREN: code = 0x64; break;
	 case SDLK_ASTERISK: code = 0x2B; break; /* *µ as ST #|@~ */
	 ...
	 case SDLK_DELETE: code = 0x53; break;
	 case 178: code = 0x29; break; /* ² as ST `£ */
...
static uint8_t Keymap_SymbolicToStScanCode_ES(const SDL_Keysym* pKeySym)
{
	uint8_t code;
	switch (pKeySym->sym)
	{
...
static uint8_t Keymap_SymbolicToStScanCode_IT(const SDL_Keysym* pKeySym)
{
	uint8_t code;
	switch (pKeySym->sym)
	{
...
static uint8_t Keymap_SymbolicToStScanCode_SE(const SDL_Keysym* pKeySym)
{
	uint8_t code;
	switch (pKeySym->sym)
	{
	 ...
	 case SDLK_z: code = 0x2C; break;
	 case SDLK_DELETE: code = 0x53; break;
	 case 167: code = 0x2B; break; /* § as ST \ */
...
static uint8_t Keymap_SymbolicToStScanCode_CH_FR(const SDL_Keysym* pKeySym)
{
	uint8_t code;
	switch (pKeySym->sym)
	{
...
static uint8_t Keymap_SymbolicToStScanCode_CH_DE(const SDL_Keysym* pKeySym)
{
	uint8_t code;
	switch (pKeySym->sym)
	{
...
static uint8_t Keymap_SymbolicToStScanCode_TR(const SDL_Keysym* pKeySym)
{
